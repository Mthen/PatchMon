...
| `zypper` | openSUSE Leap, openSUSE Tumbleweed, SLES | **Yes.** Installed RPM inventory, update detection, repository reporting, and patch runs are supported. |
...
> **SLES and OpenSUSE:** the datastreams above ship with the server and the agent maps SUSE-family hosts to the right profiles. With zypper support, enrolled SUSE hosts can run these scans normally.
