...
> **openSUSE and SLES:** the installer detects `zypper` and the agent now collects RPM inventory, updates, and repositories on SUSE-family hosts. Patch runs use `zypper refresh` and `zypper update`.
